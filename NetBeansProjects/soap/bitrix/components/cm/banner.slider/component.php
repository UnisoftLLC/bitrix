...
$arParams["IBLOCK_ID"] = trim($arParams["IBLOCK_ID"]);
	$arParams["NEWS_COUNT"] = intval($arParams["NEWS_COUNT"]);
		...
		if($prop!="")
		$arProps[] = CIBlockProperty::GetByID($prop, $arParams["IBLOCK_ID"], false)->GetNext();
		}

		foreach($arProps as $arProp){
		$arPropCode = $arProp['CODE'];
		$arrFilter = array("PROPERTY_".$arPropCode."_VALUE" => "Да");
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$arResult["ITEMS"][$arPropCode][] = $arItem;
			$arResult["ELEMENTS"][$arPropCode][] = $arItem["ID"];
		}
		$arResult["PROP"][$arPropCode]["NAME"] = $arProp["NAME"];
		}
		//pr($arResult["ITEMS"]);
...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		pr($arParams[PROPERTY_CODE]);
		foreach($arParams[PROPERTY_CODE] as $arProp){
		$arResult["ITEMS"][$arProp] = array();
		$arResult["ELEMENTS"][$arProp] = array();
		$arrFilter = array("PROPERTY_".$arProp."_VALUE"=>"Да");
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$arResult["ITEMS"][$arProp][] = $arItem;
			$arResult["ELEMENTS"][$arProp][] = $arItem["ID"];
		}
		}
	}
	else
